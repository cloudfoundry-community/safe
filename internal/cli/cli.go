...
The following options are recognized:
  ...
  -i, --signed-by X   The path to the CA that signed this certificate.
                      ...
                      Only the CA's certificate is read, so an authority
                      whose private key is kept elsewhere still works.
                      A revocation check additionally requires that the
                      path name a certificate authority.
...
The certificate's serial number is appended to the CA's revocation
list, and the CA is signed and written back, so the CA's private key
must be in the Vault.

A serial number only identifies a certificate within the authority
that issued it, and safe numbers the certificates each of its CAs
issues from one.  Revoking a certificate against a CA that did not
sign it would therefore revoke one of that CA's own certificates
instead, so safe checks the signature first and refuses.
...
Each CRL safe publishes is numbered above the one before it, as
RFC 5280 requires, so a relying party can tell which of two it should
keep.  Every write of the CA publishes one, whether or not the list
of revoked certificates changed.
